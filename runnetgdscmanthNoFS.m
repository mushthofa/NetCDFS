...
pvaltab = cell2table(cell(0, 11), 'VariableNames', {'DRUG_NAME', 'TH1', 'TH2', 'NUMPOS', 'NUMNEG', 'GDSC_AUC', 'GDSC_IC50', 'CCLE_AV', 'CCLE_AUC', 'CTRP_AV', 'CTRP_AUC'});

fpval = fopen('manthNoFS/manth_pval.csv', 'w');
ffeats = fopen('manthNoFS/allfeats.csv', 'w');
...
%     numgexp(i) = sum(cellfun(@(x) ~isempty(x)&~strcmp(x, ''), tbl.GEXsP));
...
%     numnetn(i) = sum(cellfun(@(x) ~isempty(x)&~strcmp(x, ''), tbl.NETN));
    ...
    [pmat, rowlabels, collabels,  predImp, treemdl, predlabel1, predprob1, selmat, sellabels, rf, predlabel2, predprob2] = classifyCVNoFS(gdscnet, drug, thaa);
...
%     writetable(tbl, sprintf('manthNoFS/ranking_%s.csv', drug));
    ...
    saveas(gcf, sprintf('manthNoFS/%s_predmat.png',drug));
    close all hidden;
    ...
    if(~isempty(aap) && ~isempty(aan))
        ...
        print(sprintf('manthNoFS/%s_aucvio.png', drug), '-dpng');
    end
    ...
    ylabel('Frequency');
    print(sprintf('manthNoFS/%s_aapred.png', drug), '-dpng');
    close;
    ...
    if(~isempty(ic50p) && ~isempty(ic50n))
        ...
        print(sprintf('manthNoFS/%s_ic50vio.png', drug), '-dpng');
    end
    ...
    if(sum(idxdccle)==1)
        ...
        if(allfeatav == 1)
            ...
            if(~isempty(aap) && ~isempty(aan))
                ...
                print(sprintf('manthNoFS/%s_ccleaucvio.png', drug), '-dpng');
            end
            ...
            xlabel('Activity Area');
            ylabel('Frequency');
            print(sprintf('manthNoFS/%s_predccle.png', drug), '-dpng');
    ...
    if(sum(idxdctrp)==1)
        ...
        if(allfeatav == 1)
            ...
            if(~isempty(aap) && ~isempty(aan))
                ...
                print(sprintf('manthNoFS/%s_ctrpaucvio.png', drug), '-dpng');
            end
            ...
            xlabel('Activity Area');
            ylabel('Frequency');
            print(sprintf('manthNoFS/%s_predctrp.png', drug), '-dpng');
...
writetable(confmattbl, 'manthNoFS/manth_confmat.csv');
fclose(fpval);
fclose(ffeats);
