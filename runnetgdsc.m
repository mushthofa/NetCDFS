...
%     idxpos = gdscnet.AAMat(i, :)>=0.4 | gdscnet.IC50Mat(i, :)<=0;
%     idxneg = gdscnet.AAMat(i, :)<=0.05;
    idxpos = gdscnet.IC50Mat(i, :) <= curthic50;
    idxneg = gdscnet.IC50Mat(i, :) > curthic50;
    
    gdsc_dgnet{i} = drugGeneNet(gdscnet, i, net, idxpos, idxneg);
    save -v7.3 kfgdscnet2 gdscnet gdsc_dgnet;
end
runtime_gdsc = toc;
save -v7.3 kfgdscnet2 gdscnet gdsc_dgnet runtime_gdsc;
